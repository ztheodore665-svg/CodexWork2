...
本文针对含光伏、储能和外部电网的微网购电与储能协同调度问题，综合考虑负荷需求、光伏预测误差、储能运行约束及电价变化，建立由确定性优化、机会约束和模型预测控制（MPC）构成的递进式调度模型，提出了系统的购电策略与储能调度解决方案。

\textbf{针对问题一：}
在电价、负荷及光伏预测已知的条件下，将功率数据统一转换为十分钟电量，建立以全天购电费用最小为目标的确定性线性规划模型，并考虑供需平衡、储能充放电效率、容量、功率及日循环约束。结果表明，优化后全天购电费用为35126.95元，相较无储能方案的48052.05元降低26.90\%，说明储能通过低价充电、高价放电实现跨时段能量转移，可显著降低购电成本。

\textbf{针对问题二：}
考虑负荷和光伏随日期变化且日前无法获得真实值的情况，每日0:00仅利用前14天历史净负荷计算均值与标准差，以$\mu+K\sigma$构造安全净负荷，建立机会约束日前购电模型，并利用334天真实数据进行回测。通过扫描安全系数$K$分析购电成本与供电风险的权衡，最终取$K=0.80$作为主方案，其全年总成本为1654.76万元，紧急购电量为14.67万kWh。结果表明，适当增加安全裕度能够有效降低紧急购电风险，但过度保守会增加常规购电和弃光。

\textbf{针对问题三：}
利用每日0:00、6:00、12:00和18:00不断更新的光伏预测信息，对预测数据进行跨日截断、十分钟插值和保守折减，并建立MPC滚动优化模型。在每个预测更新时间锁定已执行决策，以最新预测和当前储能状态重新优化剩余时域。结果表明，当预测折减系数$\beta=0.95$时，全年总成本为1368.87万元，较仅执行0:00计划的静态方案节省8.03\%，紧急购电量降至14.3596万kWh，说明日内预测更新能够有效修正日前预测误差，提高调度的经济性与可靠性。

\textbf{针对问题四：}
在问题二、问题三的基础上，将基础分时电价替换为随日期和时段变化的动态电价，分别建立动态电价下的机会约束日前模型和MPC滚动模型。回测结果显示，4-2方案总成本为1740.24万元，4-3 MPC方案总成本为1433.19万元；在相同4-3信息和价格条件下，MPC较0:00固定计划节省121.14万元，节省率为7.79\%。结果表明，动态价格改变了储能跨时段套利信号，而滚动预测更新能够进一步降低预测偏差带来的额外成本。

综上，本文构建了由确定性日前调度、不确定性控制、日内滚动修正到动态电价优化的微网调度框架。结果表明，储能跨时段调节可降低购电成本，安全裕度有助于平衡经济性与供电风险，滚动更新可进一步改善调度效果，为含光伏与储能微网的购电决策提供参考。
...
微网由负荷、光伏、储能和外部电网组成，调度过程同时受到电价、负荷变化、光伏预测误差以及储能容量和充放电能力的影响。由于储能具有跨时段转移能量的作用，各时段决策之间存在明显耦合；同时，日前调度只能使用决策时刻已获得的信息，预测误差可能进一步引起紧急购电或弃光。

因此，本文围绕微网购电与储能协同调度问题，在统一物理约束下，分别研究确定性日前优化、不确定性控制、光伏预测滚动修正以及动态电价条件下的调度策略，并综合评价经济成本、供电风险和光伏消纳水平\cite{olivares2014}。

\subsection{问题重述}

根据题目给出的负荷、光伏发电、储能参数及外部电网电价等数据，需要在满足微网供需平衡和储能运行约束的条件下，制定不同信息环境下的购电与储能调度方案。四个问题逐步由确定性调度扩展至预测不确定性、滚动修正和动态电价情形。

\noindent\textbf{问题1：}
在基础电价和小区负荷特征保持一致，且已知目标日光伏发电预测数据的条件下，制定当天0:00的购电与储能充放电计划。在满足供需平衡、储能容量、功率及效率约束的同时，保证0:00与24:00储电量相同，并计算全天购电费用。

\medskip

\noindent\textbf{问题2：}
在负荷和光伏发电随日期、时段变化的情况下，每天0:00仅利用此前历史信息制定日前购电计划。考虑预测误差引起的供电缺口以及5倍电价的紧急购电机制，建立具有安全裕度的调度模型，并对全年运行结果进行评价。

\medskip

\noindent\textbf{问题3：}
利用每天0:00、6:00、12:00和18:00更新的光伏预测，对尚未执行的购电和储能计划进行滚动修正。在考虑增购、退订和紧急购电费用的条件下，建立日内动态调度策略，并评价预测更新对成本和供电风险的影响。

\medskip

\noindent\textbf{问题4：}
进一步考虑外网电价随日期和时段动态变化的情形，分别在动态电价条件下重新求解问题2和问题3，分析价格波动与滚动预测修正对购电成本、紧急购电和光伏消纳的影响。


\section{总体分析}

本题属于具有时间耦合和信息不完全特征的微网多时段优化问题。储能状态会随充放电行为逐时递推，因此各十分钟时段不能独立决策，而需要在统一的SOC、容量和充放电功率约束下进行联合优化。

四问的核心区别在于决策时可获得的信息不同。问题1中，当天负荷、光伏预测和电价均视为已知，可建立确定性线性规划作为基准；问题2中，每天0:00只能利用历史数据估计当天净负荷，需要在日前购电成本与紧急购电风险之间进行权衡；问题3进一步利用日内不断更新的光伏预测，通过滚动优化修正尚未执行的计划；问题4则加入动态电价，使储能调度同时受到预测误差和价格信号的影响。
...
\text{动态电价扩展}
\]
的思路逐步建模。四问统一采用十分钟电量作为决策单位，并使用相同的储能状态方程、容量边界、充放电功率限制和日循环约束。

在模型评价中，本文严格区分计划阶段和真实执行阶段：计划阶段仅使用决策时已获得的信息，真实负荷和实际光伏只在回测阶段用于计算执行结果、紧急购电、弃光及最终成本，从而避免未来信息泄漏。

此外，通过无储能对照、安全系数$K$扫描、固定计划与MPC对照以及预测折减系数$\beta$敏感性分析，分别评价储能经济收益、风险--成本权衡、预测更新价值和参数保守程度的影响\cite{boyd2004}。
...
3 & 0:00、6:00、12:00、18:00光伏预报
& MPC滚动规划
& 固定0:00计划对照，检验预测更新收益\\

4 & 问题二、三信息与动态电价
& 动态电价机会约束与MPC
& 区分价格与滚动修正作用，检验$K,\beta$敏感性\\
\bottomrule
\end{tabularx}
\end{table}
...
问题一使用题目给定的单日负荷、光伏预测和基础分时电价。程序将功率统一乘以$1/6$转换为十分钟电量，并按照$t=0,1,\ldots,143$建立全天离散时段。储能初始电量为$6000\,\mathrm{kWh}$，上下界为
$1200$--$10800\,\mathrm{kWh}$，充放电效率均为$0.90$，
额定充放电功率上限为$5000\,\mathrm{kW}$，
对应单个十分钟时段最大充放电量为$833.3333\,\mathrm{kWh}$。已知输入为$(p_t,E_{\mathrm{load},t},E_{\mathrm{pv},t})$，决策变量为$(E_{\mathrm{grid},t},E_{\mathrm{ch},t},E_{\mathrm{dis},t})$，状态变量为$S_t$。
...
\tag{9}
\end{equation}
式（9）的不等式允许右侧未被负荷或储能吸收的光伏自然弃光；在不允许反送电且弃光不计入目标时，引入弃光变量并写成等式与当前写法等价。式（4）描述充放电效率造成的SOC跨期耦合，式（5）限制设备功率，式（6）限制容量并固定日初、日末SOC。因正电价和效率损耗均为正，模型不会通过同时充放电获得目标值下降；日循环边界则排除了透支期末SOC的虚假收益。
...
图~\ref{fig:q1dispatch}和图~\ref{fig:q1soc}共同表明，储能在低价或光伏富余时段充电，在高价或光伏不足时段放电；SOC始终位于1200--10800 kWh且首尾相接。与无储能基准相比，购电费用由48052.05元降至35126.95元，减少12925.10元，降幅为26.90\%，说明储能跨时段转移在该基准日具有明显的经济收益。
...
容量和效率敏感性实验显示，扩大容量或者提高效率都可以降低购电费用，但容量收益会随着规模扩大而递减；其原因是可利用的低价—高价价差和可转移电量有限。容量关系见图~\ref{fig:q1capacity}，完整扫描数据随支撑材料提交。
...
微扰1 kWh负荷得到的影子价格为：02:00和12:00分别为0.4293和0.4411元/kWh，与外网电价一致；19:00为1.2561元/kWh，低于外网电价1.3523元/kWh，这表明储能在该高价时段承担了部分边际需求。
...
问题二将问题一推广到了全年历史回测，并限制每天0:00只能使用前14天历史净负荷。以$\mu_{d,t}+K\sigma_{d,t}$构造安全净负荷后，日前计划不再使用当天真实数据；真实负荷和光伏仅用于回测状态机。$K$过小会减少日前购电却增加紧急购电，$K$过大则相反。有限网格结果选定$K=0.80$，对应总成本1654.76万元、紧急购电量14.67万kWh；该选择是样本内的成本—风险折中，不代表全天联合可靠性。

\subsubsection{模型准备}

\paragraph{1. 数据预处理}

问题二在每天0:00制定全天计划，决策时不能使用当天的真实负荷和光伏。对日期$d$，用前$W=14$天同一时段的净负荷样本计算
\begin{equation}
...
在每天0:00信息集固定后，历史统计量可以将不确定的供电要求转化为确定的安全净负荷，因此日前决策仍然可以采用线性规划。考虑到当天真实数据只能在回测阶段使用，本文采用“机会约束日前规划+真实状态机回测”的两阶段算法：前一阶段生成不依赖当天真实数据的计划，后一阶段检验计划在真实负荷和实际光伏下的可执行性。该处理既保持机会约束模型的可解性，又避免使用未来信息造成结果偏乐观。
...
\end{equation}
\paragraph{模型性质与执行逻辑}
式（13）的日前目标只计计划普通购电费；式（14）--（14d）把真实净负荷与计划之间的缺口交给储能状态机处理，储能受功率和SOC边界限制后的剩余缺口记为紧急购电，剩余富余量标记为弃光。这样，优化阶段与执行阶段的职责分离，且紧急购电不会被日前模型提前“看见”。

因此，日前计划和真实执行分别由预测数据与实际数据决定，从而避免信息泄漏。
...
\begin{tabular}{lrrr}
\toprule
\textbf{策略} & \textbf{总成本（万元）} & \textbf{紧急购电（万kWh）} & \textbf{节省比例}\\
\midrule
...
在334天样本上，预测MAE由0:00的207.47 kW降到18:00的7.20 kW，新增信息的误差修正空间随发布时间缩小；但本文未对更高频更新进行独立对照，因此只能认为现有四个断面在本数据和费用口径下足以支撑主方案，不能推出普遍最优性。

\paragraph{成本构成与典型日期核验}

主方案的实际购电、偏差和紧急购电费用分别占总成本的93.19\%、2.62\%和4.19\%，主要收益来自购电与储能时序的改善；但光伏快速变化或储能功率受限时仍会产生紧急购电。
...
&
\multicolumn{1}{c}{\textbf{(kWh)}} &
\multicolumn{1}{c}{\textbf{(元)}} &
\multicolumn{1}{c}{\textbf{(kWh)}} &
\multicolumn{1}{c}{\textbf{(kWh)}}\\
\midrule
...
紧急购电主要集中于清晨光伏爬坡阶段；而且不同季节的发生区间随日出时间和爬坡速度变化。完整轨迹根据支撑材料提交。

\paragraph{改进方案的消融实验}

为判断风险是否主要来自光伏预报的物理不合理片段，方案A将日出前、日落后的预测置零，并将6:30--8:30的晨间爬坡预测乘以0.75。在$\beta=0.95$下，方案A把紧急购电量从14.3596万kWh下降到4.7014万kWh，但总成本由1368.87万元升高到1379.37万元。进一步在方案A上把5:00--8:00的储能最低电量由1200提高到2000 kWh，得到方案A+B总成本1381.24万元，紧急购电量仍为4.7014万kWh。

物理清洗将紧急购电量下降到4.7014万kWh，但总成本升高到1379.37万元；叠加晨间SOC下限后成本进一步升高到1381.24万元且风险未继续下降。图~\ref{fig:q3cleancompare}表明，当前改进的主要代价是增加常规购电，限制因素更可能是预测误差和放电功率。
...
问题四在问题二、三的信息结构上引入日期—时段动态电价。4-2用于评价动态价格下的机会约束日前调度，4-3用于评价动态价格下的MPC调度。4-2与4-3的跨模型差异不能单独归因于滚动机制，因此另设相同4-3信息和价格下的0:00固定计划基准。回测中4-2和4-3总成本分别为1740.24万元和1433.19万元；同一4-3模型中，MPC与固定计划相比节省121.14万元，节省率7.79%。
...
\end{equation}
\paragraph{模型性质与结算逻辑}
式（24）和式（27）分别是最小化动态价格下的日前购电成本和滚动结算成本；式（25）将真实缺口按5倍动态电价结算。式（26）保证预测供需覆盖，式（28）--（30）保证实际执行轨迹满足能量守恒，并分别识别紧急购电与弃光。固定$K$或$\beta$后，历史统计量和价格均为已知参数，模型仍是连续线性规划；偏差变量的正成本使$u^+$与$u^-$不会同时为正。
...
4-3相较于4-2将紧急购电量下降到38.79\%、紧急购电成本下降到36.45\%，弃光量下降69.86\%；但紧急购电天数由246天增加到334天，说明风险总量下降并不等于发生天数下降。与同一4-3模型的0:00固定计划相比，MPC节省121.14万元，7.79\%的节省率可用于识别滚动机制的边际收益。
...
\begin{tabular}{ccrrr}
\toprule
\textbf{模型} & \textbf{参数} & \textbf{总成本（万元）} & \textbf{紧急购电量（万kWh）} & \textbf{备注}\\
\midrule
...
4-2的较高弃光量与日前安全裕度占用购电和储能空间有关；4-3利用新预测减少了不必要的购电和充电，因此弃光量下降。该解释是对334天样本的统计归纳，不外推至其他误差分布。
...
在题目数据和逐日闭环边界下，问题一费用下降26.90\%；问题二$K=0.80$的334天总成本为1654.76万元；问题三$\beta=0.95$的MPC总成本为1368.87万元，与静态计划相比节省8.03\%；问题四4-2和4-3总成本分别为1740.24万元和1433.19万元，4-3较同模型固定计划节省7.79\%。这些结果支持“适度保守预测+滚动更新”在本样本上的成本—风险折中，但不外推至其他样本、价格机制或全年连续SOC运行。
...
\caption{附件说明表}
\label{tab:appendix_support}
\begin{tabularx}{\textwidth}{p{4.5cm}X}
\toprule
\textbf{文件} & \textbf{说明}\\
\midrule
q1\_code & 问题一线性规划求解与灵敏度分析程序\\
q1\_figures & 问题一调度策略与储能 SOC 曲线图表\\
q2\_code & 问题二机会约束日前优化与回测主程序\\
q2\_figures & 问题二安全裕度扫描与典型日分析图表\\
q3\_code & 问题三 MPC 滚动调整与消融实验程序\\
q3\_figures & 问题三误差分析、策略对比及前沿图表\\
q4\_code & 问题四动态电价优化与结果分析程序\\
q4\_figures & 问题四极端价差日套利与费用对比图表\\
result1.xlsx & 问题一计划购电与储能调度结果表\\
result2.xlsx & 问题二日前计划与紧急购电记录表\\
result3.xlsx & 问题三调整购电与紧急购电记录表\\
result4-2.xlsx & 动态电价下问题二日前调度结果表\\
result4-3.xlsx & 动态电价下问题三滚动调整结果表\\
代码运行说明.txt & 运行环境配置与代码一键复现说明\\
\bottomrule
\end{tabularx}
\end{table}
