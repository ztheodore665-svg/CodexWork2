...
程序采用Python读取附件1至附件4，并将每一天的数据整理为长度为144的数组。问题二和4-2先根据目标日前14天样本计算净负荷统计量，再调用线性规划求解器；问题三和4-3先对附件3预报完成日期向前填充、跨日截断和线性插值，再按四个发布时间依次求解。每次滚动求解只保留下一个执行区间，已执行区间的购电量和储能状态不再修改。
