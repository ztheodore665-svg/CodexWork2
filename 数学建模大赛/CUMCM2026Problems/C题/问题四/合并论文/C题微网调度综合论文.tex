...
本文分别对储能容量、充放电效率、安全系数$K$以及预测折减系数$\alpha$和$\beta$进行单因素敏感性分析。问题一中，容量和效率变化用于判断储能套利收益是否依赖某一组设备参数；问题二中，$K$扫描用于观察普通购电、紧急购电和弃光之间的风险--成本权衡；问题三和问题四中，$\alpha$、$\beta$扫描用于检验预测保守程度对MPC成本和可靠性指标的影响。各问题的原始扫描数据和中间结果保存在支撑材料中，正文仅展示具有解释力的曲线和汇总表。

敏感性结果表明，储能容量和效率提高通常能够扩大可行调度空间，但边际收益会逐步减小；安全系数过低会增加紧急购电风险，过高则可能增加普通购电和弃光；预测折减系数存在使综合成本较低的中间区间。因此，本文报告的$K=0.80$、$\alpha=0.95$和$\beta=0.95$均应理解为给定样本和费用口径下的实验选择，而不是脱离数据的普适最优参数。

\subsection{结果合理性检验}

程序采用Python读取附件1至附件4，并将每一天的数据整理为长度为144的数组。问题二和4-2先根据目标日前14天样本计算净负荷统计量，再调用线性规划求解器；问题三和4-3先对附件3预报完成日期向前填充、跨日截断和线性插值，再按四个发布时间依次求解。每次滚动求解只保留下一个执行区间，已执行区间的购电量和储能状态不再修改。本文的核心算法节选和支撑材料索引统一列于文末附录。
...
针对上述局限，后续可从以下方面改进：
\begin{enumerate}
    \item 将日前普通购电计划与紧急购电补救统一写成两阶段随机规划或分布鲁棒优化，并显式刻画负荷、光伏和价格误差的联合分布；
    \item 将每日固定的始末储电量改为全年连续SOC传递，同时加入跨日能量价值和初始状态不确定性；
    \item 将光伏预测折减系数、安全系数和偏差费用参数置于独立验证集上标定，避免仅依靠同一样本进行参数选择；
    \item 引入日前价格预测、实时价格更新和滚动风险预算，使动态电价场景更接近实际在线调度。
\end{enumerate}

\subsection{模型的推广}

本文的建模框架还可推广到含风电、可控负荷、需求响应和多储能设备的综合能源系统。只要将新增设备的功率边界、能量状态和转换效率写成线性或分段线性约束，便可继续使用线性规划和MPC求解。对于多个微网之间的能量交易，可将外部购电变量扩展为节点间潮流变量，并加入网络容量约束；对于价格和可再生能源均具有不确定性的场景，则可将本文的历史窗口和滚动更新机制嵌入场景随机规划或分布鲁棒MPC。
...
\texttt{q2\_k\_scan.png} & 问题二安全系数扫描结果图。\\
\texttt{q3\_clean\_compare.png} & 问题三不同预测处理方案对比图。\\
\texttt{q4\_cost.png} & 问题四动态电价下的成本对比图。\\
\end{longtable}
