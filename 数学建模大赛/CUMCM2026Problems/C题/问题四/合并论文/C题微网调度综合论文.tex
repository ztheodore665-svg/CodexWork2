...
\textbf{文件} & \textbf{说明}\\
\midrule
\texttt{q1\_code} & 问题一确定性线性规划、无储能对照、敏感性分析与结果校验程序。\\
\texttt{q1\_figures} & 问题一调度轨迹、SOC及容量和效率敏感性等图形结果。\\
\texttt{q2\_code} & 问题二机会约束日前规划、安全系数扫描与全年真实回测程序。\\
\texttt{q2\_figures} & 问题二安全系数、回测指标和典型日期分析图形结果。\\
\texttt{q3\_code} & 问题三光伏预测插值、MPC滚动调度、预测清洗及消融实验程序。\\
\texttt{q3\_figures} & 问题三策略对比、预测误差、敏感性和典型日期图形结果。\\
\texttt{q4\_code} & 问题四动态电价4-2日前规划、4-3 MPC及结果分析程序。\\
\texttt{q4\_figures} & 问题四动态电价、成本、紧急购电、弃光和储能套利图形结果。\\
\texttt{result1.xlsx} & 问题一最终求解结果及调度明细文件。\\
\texttt{result2.xlsx} & 问题二全年日前计划、真实回测与汇总结果文件。\\
\texttt{result3.xlsx} & 问题三MPC滚动调度及预测处理方案结果文件。\\
\texttt{result4-2.xlsx} & 问题四4-2动态电价机会约束日前结果文件。\\
\texttt{result4-3.xlsx} & 问题四4-3动态电价MPC结果文件。\\
\texttt{代码运行说明.txt} & 支撑材料的运行环境、程序执行顺序和结果文件说明。\\
\bottomrule
\end{tabularx}
\end{table}
