...
\texttt{solve_q1.py}、\texttt{validation_q1.py} & 问题一求解、无储能对照、敏感性、影子价格及结果有效性检验代码。\\
\texttt{q1_*.png} & 问题一储能调度、能量平衡、SOC、经济性和参数分析图。\\
\texttt{solve_q2.py}、\texttt{refine_k.py}、\texttt{run_dynamic_mild.py} & 问题二全年机会约束求解、安全系数$K$细扫和动态安全裕度对照实验代码。\\
\texttt{q2_*.png} & 问题二固定$K$粗扫、局部细扫和动态安全裕度分析图。\\
\texttt{solve_q3.py} & 问题三原版光伏预测更新与MPC滚动购电求解代码，正文主方案来源。\\
\texttt{solve_q3_clean.py}、\texttt{solve_q3_clean_ab.py} & 问题三预测物理清洗和晨间SOC下限消融实验代码。\\
\texttt{result3.xlsx}、\texttt{q3_*.csv}、\texttt{q3_*.png} & 问题三策略对比、预测误差、紧急购电、成本结构、典型日期和消融实验材料。\\
\texttt{solve_q4_2.py}、\texttt{solve_q4_3.py} & 问题四4-2动态电价机会约束和4-3动态电价MPC求解代码。\\
\texttt{q4_analysis_only.py}、\texttt{q4_plot.py}、\texttt{q4_*.csv}、\texttt{q4_*.png} & 问题四全周期汇总、极端价差、弃光、紧急购电、参数敏感性及动态电价分析材料。\\
