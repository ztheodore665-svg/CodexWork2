...
本表列出本文四个问题所使用的本地实验材料。表中每行仅列出一个文件名，并为该文件给出唯一说明；程序、分析结果、图形和提交文件与正文中的问题一至问题四分别对应。原始附件文件均保留在原目录中。
...
\texttt{q1\_capacity.png} & 问题一储能容量敏感性分析图。\\
\texttt{q1\_dispatch.png} & 问题一电价与储能充放电调度图。\\
\texttt{q1\_efficiency.png} & 问题一储能充放电效率敏感性分析图。\\
\texttt{q1\_shadow.png} & 问题一储能容量约束影子价格分析图。\\
\texttt{q1\_soc.png} & 问题一储能荷电状态变化图。\\
\texttt{q1\_validity.png} & 问题一能量守恒与约束有效性检验图。\\
\texttt{q2\_dynamic.png} & 问题二动态安全裕度方案分析图。\\
\texttt{q2\_k\_fine.png} & 问题二安全系数$K$局部细扫分析图。\\
\texttt{q2\_k\_scan.png} & 问题二安全系数$K$粗扫描分析图。\\
\texttt{q3\_clean\_compare.png} & 问题三预测清洗方案对比图。\\
\texttt{q3\_compare.png} & 问题三原版与滚动方案综合对比图。\\
\texttt{q3\_correlation.png} & 问题三预测误差与紧急购电相关性图。\\
\texttt{q3\_cost\_structure.png} & 问题三成本结构分析图。\\
\texttt{q3\_curve\_feb.png} & 问题三二月典型日期调度曲线图。\\
\texttt{q3\_curve\_jun.png} & 问题三六月典型日期调度曲线图。\\
\texttt{q3\_curve\_sep.png} & 问题三九月典型日期调度曲线图。\\
\texttt{q3\_curve\_dec.png} & 问题三十二月典型日期调度曲线图。\\
\texttt{q3\_emergency.png} & 问题三紧急购电分析图。\\
\texttt{q3\_error.png} & 问题三预测误差分布分析图。\\
\texttt{q3\_pareto.png} & 问题三成本与风险的Pareto分析图。\\
\texttt{q3\_sensitivity.png} & 问题三参数敏感性分析图。\\
\texttt{q4\_2\_emergency.png} & 问题四4-2紧急购电分析图。\\
\texttt{q4\_3\_emergency.png} & 问题四4-3紧急购电分析图。\\
\texttt{q4\_arbitrage\_buy.png} & 问题四动态电价下购电套利分析图。\\
\texttt{q4\_arbitrage\_compare.png} & 问题四固定计划与MPC套利效果对比图。\\
\texttt{q4\_arbitrage\_soc.png} & 问题四动态电价下储能SOC套利轨迹图。\\
\texttt{q4\_beta.png} & 问题四预测折减参数$\beta$敏感性分析图。\\
\texttt{q4\_cost.png} & 问题四不同方案成本结构分析图。\\
\texttt{q4\_curtail.png} & 问题四不同方案弃光量分析图。
\end{longtable}
