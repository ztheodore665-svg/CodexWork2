...
程序采用Python读取附件1至附件4，并将每一天的数据整理为长度为144的数组。问题二和4-2先根据目标日前14天样本计算净负荷统计量，再调用线性规划求解器；问题三和4-3先对附件3预报完成日期向前填充、跨日截断和线性插值，再按四个发布时间依次求解。每次滚动求解只保留下一个执行区间，已执行区间的购电量和储能状态不再修改。统一参数和变量边界见附录A，算法与输出字段的复核口径见附录B和附录C。
...
本文正文中的数值、图形和表格均应能够由本地实验程序生成的结果文件复核。文件名按照题目要求和本地项目中的输出约定列出如下。

\begin{table}[H]
\centering
\small
\renewcommand\arraystretch{1.25}
\caption{四问结果文件及主要复核内容}
\label{tab:appendix_files}
\begin{tabularx}{\textwidth}{p{3.2cm}p{3.0cm}X}
\toprule
\textbf{结果文件} & \textbf{对应方案} & \textbf{主要复核内容}\\
\midrule
\texttt{result1.xlsx} & 问题一 & 单日十分钟购电量、充放电量、储能状态及无储能对照\\
\texttt{result2.xlsx} & 问题二 & 334天日前计划、紧急购电、弃光和$K$敏感性汇总\\
\texttt{result3.xlsx} & 问题三 & 四个预测发布时间下的滚动执行轨迹、偏差费用和$\alpha$敏感性\\
\texttt{result4-2.xlsx} & 问题四4-2 & 动态电价机会约束日前计划及$K$敏感性结果\\
\texttt{result4-3.xlsx} & 问题四4-3 & 动态电价MPC轨迹、偏差结算和$\beta$敏感性结果\\
\bottomrule
\end{tabularx}
\end{table}

复核任一结果文件时，建议按以下顺序执行：首先确认日期范围和每一天的144个时段均存在；其次确认普通购电、充电和放电量均为非负；再次由式（4）重新递推储能状态并核对上下界与日循环边界；最后重新汇总成本、紧急购电量、紧急购电天数和弃光量。对于问题三和4-3，还应检查更新时刻之前的执行轨迹在后续重优化中保持不变，以确认滚动模型没有使用未来信息。
