...
\begin{abstract}
针对题目给出的含光伏、储能和外部电网的微网系统，本文将四个问题统一为“日前计划--日内执行--信息更新--动态结算”的递进式调度框架。首先，将功率数据按十分钟离散并转换为电量，在满足供需平衡、储能容量、充放电功率、效率和日循环边界的条件下，建立问题一的确定性线性规划模型。其次，针对实际负荷和光伏存在预测误差的情形，使用过去14天同一时段净负荷的均值和无偏标准差构造安全裕度，建立问题二的机会约束日前模型，并通过真实数据回测选择安全系数。再次，将每天0:00、6:00、12:00和18:00发布的光伏预报进行跨日截断和线性插值，在计划偏差结算规则下建立问题三的模型预测控制（MPC）模型。最后，将固定分时电价替换为按日期和时段变化的动态电价，得到问题四的动态电价机会约束模型和动态电价MPC模型。

问题一的基准日结果显示，储能优化使购电费用由无储能方案的48052.05元降至35126.95元，下降26.90\%。在2025年2月1日至12月31日的334天回测中，问题二固定安全系数$K=0.80$的总成本为1654.76万元，问题三的主方案（预测折减系数$\alpha=0.95$）总成本为1368.87万元，相对仅执行0:00计划的基准节省8.03\%。在动态电价场景下，问题四的4-2和4-3总成本分别为1740.24万元和1433.19万元；在相同4-3模型下，MPC相对0:00固定计划节省121.14万元，节省率为7.79\%。结果表明，储能跨时段转移、风险裕度和预测更新共同决定经济性与供电可靠性；所有结论均限定在题目数据、日循环边界和本文回测口径内。
...
本文坚持三个原则。第一，所有决策变量统一使用十分钟电量，避免在目标函数、能量平衡和储能递推中混用功率与电量。第二，严格区分决策时可获得的信息和事后评价使用的真实数据，避免以真实数据构成“完美预测”。第三，对于不同信息集或不同价格结构下的方案，只作描述性比较；评价MPC本身的边际作用时，使用同一模型下的固定计划基准。

\section{数据处理、参数与统一符号}

\subsection{时间离散和电量换算}
