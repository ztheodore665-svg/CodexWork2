...
for T_k in [35, 71, 107]:
    plan = solve_mpc_adj(T_k, S_current, E_plan[T_k:],
                         load[T_k:], pv_pred[T_k:], prices_day[T_k:])
    execute_and_update_soc(plan, T_k)
\end{lstlisting}

\section*{支撑材料索引}

支撑材料 ZIP 中保留完整的模块化源程序、结果文件和必要的中间结果。下表只列出用于复核模型实现和主要结论的核心材料，不重复列出全部图片；每行对应一个文件。
...
\bottomrule
\endlastfoot
\texttt{solve\_q1.py} & 问题一确定性线性规划完整源程序。\\
\texttt{validation\_q1.py} & 问题一对照、敏感性和有效性检验源程序。\\
\texttt{solve\_q2.py} & 问题二机会约束日前规划和全年回测源程序。\\
\texttt{refine\_k.py} & 问题二安全系数局部细扫源程序。\\
\texttt{run\_dynamic\_mild.py} & 问题二动态安全裕度对照源程序。\\
\texttt{solve\_q3.py} & 问题三原版预测更新与 MPC 完整源程序。\\
\texttt{solve\_q3\_clean.py} & 问题三预测物理清洗方案源程序。\\
\texttt{solve\_q3\_clean\_ab.py} & 问题三清洗加晨间 SOC 下限消融源程序。\\
\texttt{analyze\_q3\_extra.py} & 问题三原版结果分析源程序。\\
\texttt{analyze\_q3\_clean\_extra.py} & 问题三清洗方案结果分析源程序。\\
\texttt{solve\_q4\_2.py} & 问题四4-2动态电价机会约束源程序。\\
\texttt{solve\_q4\_3.py} & 问题四4-3动态电价 MPC 源程序。\\
\texttt{q4\_analysis\_only.py} & 问题四全周期中间结果汇总源程序。\\
\texttt{q4\_plot.py} & 问题四代表性结果绘图源程序。\\
\texttt{result1.xlsx} & 问题一最终结果文件。\\
\texttt{result2.xlsx} & 问题二最终结果文件。\\
\texttt{result3.xlsx} & 问题三原版最终结果文件。\\
\texttt{q3\_clean\_result3.xlsx} & 问题三预测清洗方案结果文件。\\
\texttt{q3\_clean\_ab\_result3.xlsx} & 问题三消融方案结果文件。\\
\texttt{result4-2.xlsx} & 问题四4-2最终结果文件。\\
\texttt{result4-3.xlsx} & 问题四4-3最终结果文件。\\
\texttt{q3\_forecast\_error\_detail.csv} & 问题三原版大篇幅预测误差明细。\\
\texttt{q3\_emergency\_matrix.csv} & 问题三原版全年紧急购电矩阵。\\
\texttt{q3\_sensitivity\_full.csv} & 问题三原版参数敏感性完整结果。\\
\texttt{q3\_clean\_forecast\_error\_detail.csv} & 问题三清洗方案预测误差明细。\\
\texttt{q3\_clean\_emergency\_matrix.csv} & 问题三清洗方案全年紧急购电矩阵。\\
\texttt{q3\_clean\_ab\_forecast\_error\_detail.csv} & 问题三消融方案预测误差明细。\\
\texttt{q3\_clean\_ab\_emergency\_matrix.csv} & 问题三消融方案全年紧急购电矩阵。\\
\texttt{q4\_2\_emer\_matrix.csv} & 问题四4-2全年紧急购电矩阵。\\
\texttt{q4\_3\_emer\_matrix.csv} & 问题四4-3全年紧急购电矩阵。\\
\texttt{q4\_2\_curt\_matrix.csv} & 问题四4-2全年弃光矩阵。\\
\texttt{q4\_3\_curt\_matrix.csv} & 问题四4-3全年弃光矩阵。\\
\texttt{q4\_2\_K\_sensitivity.csv} & 问题四4-2安全系数敏感性结果。\\
\texttt{q4\_3\_beta\_sensitivity.csv} & 问题四4-3预测折减参数敏感性结果。\\
\texttt{q4\_extreme\_spread\_day.csv} & 问题四4-2极端价差日中间结果。\\
\texttt{q4\_3\_extreme\_spread\_day.csv} & 问题四4-3极端价差日中间结果。\\
\texttt{q4\_curtailment\_hourly.csv} & 问题四按小时弃光汇总结果。\\
\texttt{q1\_capacity.png} & 问题一容量敏感性代表性图表。\\
\texttt{q2\_k\_scan.png} & 问题二安全系数扫描代表性图表。\\
\texttt{q3\_clean\_compare.png} & 问题三方案对比代表性图表。\\
\texttt{q4\_cost.png} & 问题四动态电价成本结构代表性图表。\\
\end{longtable}
