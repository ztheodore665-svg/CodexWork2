...
进一步采用单因素实验考察设备参数的边际作用。将储能最大容量取8000、10000、12000、14000和16000\kwh重新求解，购电费用随容量增加而下降，但下降斜率逐渐变小，表明容量扩张存在边际收益递减。将充放电效率同步取0.80、0.85、0.90和0.95时，效率提高会降低购电费用，原因是同样的跨时段转移损耗更小。容量敏感性曲线见图~\ref{fig:q1capacity}，效率扫描的完整数据随支撑材料提交。
...
为考察分时安全裕度的作用，另设一个只依赖历史光伏均值的温和动态规则：在历史平均光伏较高的时段降低$K$，在高电价时段提高$K$，其余时段保持基准值。以$K_0=0.85$为例，动态规则将弃光量从固定$K=0.85$的591.20万kWh降至502.32万kWh，但紧急购电量升至28.22万kWh，总成本达到1703.05万元。该规则并未在本数据集上同时改善经济性和可靠性，说明仅凭历史平均光伏推断当日天气仍然不足；其逐日统计数据随支撑材料提交。
...
图~\ref{fig:q3sensitivity}从总成本曲线展示上述结论：保守程度存在成本谷值；若单独追求最低紧急购电量，应选择更小的$\alpha$，但这会牺牲经济性。成本--风险权衡的完整数据见支撑材料。
...
以2025-03-20为例，紧急购电主要集中在6:00--13:50的光伏爬坡和预测误差阶段，分段电量为510.3796、69.5725、403.3834、21.9267和58.2846 kWh；2025-12-21则主要发生在7:00--12:00，说明季节性日出时间和光伏爬坡速度会改变风险时段。四个代表日的完整负荷、光伏、购电与储能轨迹随支撑材料提交。
...
附件4中日内价差最大的样本日为2025年8月15日，最低电价约0.1119元/kWh，最高电价约1.6114元/kWh。该日调度轨迹呈现低价充电、高价放电，说明动态电价确实进入了储能决策。对$K=0.70,0.75,0.80,0.84,0.90$的4-2敏感性分析显示，$K=0.80$在测试网格上总成本最低；$K=0.84$的紧急购电量更低但总成本略高，可作为可靠性优先的备选。对$\beta=1.00,0.98,0.95,0.90$的4-3敏感性分析显示，$\beta=0.95$取得最低总成本和最高节省率，$\beta=0.90$的紧急购电量更低但保守购电成本上升，正文以汇总表和成本图呈现，完整扫描数据随支撑材料提交。
...
为了避免只用总成本解释动态电价模型，进一步按小时统计紧急购电。4-2的紧急购电量为37.0508万kWh，4-3为14.3723万kWh；但4-3的发生天数更多，说明总量减少并不意味着每一天都完全消除了风险。两种方案的小时紧急购电和弃光时段统计见支撑材料。
...
根据附件4逐日计算最高电价与最低电价之差，2025-08-15是回测区间内价差最大的样本日。图~\ref{fig:q4arbitrage}显示，储能SOC在低价时段上升，在高价时段下降；相应的净购电轨迹和动态价格对比数据见支撑材料。由式（24）至式（27）可知，这种轨迹并非人为设定，而是动态价格进入目标函数后，在线性约束下产生的最优结果。
...
表~\ref{tab:q4sens}列出了4-2安全系数和4-3预测折减系数的关键敏感性结果。$K$由0.70增加到0.80时，紧急购电成本下降速度快于普通购电成本上升速度，总成本下降；继续增大到0.90时，普通购电成本的增加抵消了风险收益。$\beta$从1.00降低到0.95时，滚动预测的保守修正改善了综合成本；继续降低到0.90虽使紧急购电量进一步下降，但过度保守使总成本回升。完整敏感性表保存在支撑材料中。
