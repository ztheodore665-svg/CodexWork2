# -*- coding: utf-8 -*-
"""路口4 调度算法：Webster 最优配时执行器（单路口, 4 相位）。

方法（平台 scheme_1 核心算法）：
  按进口流量与饱和流(1800 veh/h/车道)求关键流量比 Y，算最优周期 C0=(1.5L+5)/(1-Y)，
  有效绿按 Y 分配；本路口低-中饱和，最优周期约 38s。
建议相位时长（黄灯不变）：相位0(-E2/-E3)=16s 相位1(黄)=3 相位2(-E1/E0)=16 相位3(黄)=3
效果(900s 仿真对比基线)：平均等待 109.6 -> 27.3 s（-75%），最大排队 13->9。
...
def main():
    ...
    traci.close()
    if wait_n:
        print(f"[路口4] 末段平均在网等待 ≈ {wait_sum / wait_n:.1f} s"
              f"（Webster 配时，基线约 109.6s）")
