# -*- coding: utf-8 -*-
"""路口1 调度算法：Webster 最优配时执行器（单路口, 6 相位）。

方法（平台 scheme_1 核心算法）：
  按进口流量与饱和流(1800 veh/h/车道)求关键流量比 Y，算最优周期 C0=(1.5L+5)/(1-Y)，
  有效绿按 Y 分配；本路口低-中饱和，最优周期≈39-40s。
建议相位时长（黄灯不变）：
  相位0(直行组)=10s  相位1(黄)=3  相位2(转向组)=10  相位3(黄)=3  相位4(直行组)=10  相位5(黄)=3
效果(900s 仿真对比基线)：平均等待 217.9 -> 36.0 s（-83%），最大排队 19->8。
...
SUGGEST = {0: 10, 1: 3, 2: 10, 3: 3, 4: 10, 5: 3}
CFG = "demo_1.sumocfg"
...
def main():
    ...
    traci.close()
    if wait_n:
        print(f"[路口1] 末段平均在网等待 ≈ {wait_sum / wait_n:.1f} s"
              f"（Webster 配时，基线约 217.9s）")
